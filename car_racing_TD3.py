# car_racing_TD3.py
# TD3 for custom CarRacing (continuous steer/gas/brake/pit)
...
class TD3CarRacingAgent:
    def __init__(self, env: CarRacing, cfg: TD3Config, device=None):
        ...
        # State normalization (same idea as SAC)
        ...
        # Reward shaping knobs (same structure as SAC/PPO/DQN so results are comparable). :contentReference[oaicite:0]{index=0}
    ...
    def select_action(self, state: np.ndarray, deterministic: bool = False, noise_scale: float | None = None):
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            next_actions = self._rescale_action(next_actions)
        ...
        if self.total_it % self.cfg.policy_delay == 0:
            ...
            actor_actions_env = self._rescale_action(actor_actions)
...
def main():
    ...
    if len(episode_returns) > 0:
        ...
        plt.xlabel("Episode")
        plt.ylabel("Return")
        plt.title("TD3 on Custom CarRacing")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig("td3_car_racing_returns.png", dpi=150)
        plt.close()
    else:
